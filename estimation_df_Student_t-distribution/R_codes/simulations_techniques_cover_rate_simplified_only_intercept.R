### V5 model: V3 but adding (i) a randomized df per dataset; (ii) an aletrenative tmodel with mu fixed at 0 (=> estimatedtbis); (iii) set.seed(j+i*N,kind="default",normal.kind="default",sample.kind="default")
...
name<-paste0("rep",rep,"_N",N,"_dataSize",dataSize,"_",Nchains,"chains","_Rhat",Rhat.max,"_neff",neff.min)
name_reduced<-paste0("_N",N,"_dataSize",dataSize,"_",Nchains,"chains","_Rhat",Rhat.max,"_neff",neff.min)
...
i<-0
mean_estimates<-matrix(NA,nrow=rep,ncol=N)
se_estimates<-matrix(NA,nrow=rep,ncol=N)
res_Bayest<-NULL
res_Bayestbis<-NULL
res_BayesGaus<-NULL
res_MCMC_dfs_t<-NULL
res_MCMC_dfs_tbis<-NULL

data_cover_rate_gauss_semult<-matrix(NA,nrow=rep,ncol=N)
data_cover_rate_theort1_semult<-matrix(NA,nrow=rep,ncol=N)
data_cover_rate_theort1_sesdt<-matrix(NA,nrow=rep,ncol=N)
data_cover_rate_theort2_semult<-matrix(NA,nrow=rep,ncol=N)
data_cover_rate_theort2_sesdt<-matrix(NA,nrow=rep,ncol=N)
data_cover_rate_estimatedt_semult<-array(NA,dim=c(rep,N,Npvalues))
data_cover_rate_estimatedt_sesdt<-array(NA,dim=c(rep,N,Npvalues))
data_cover_rate_estimatedtbis_semult<-array(NA,dim=c(rep,N,Npvalues))
data_cover_rate_estimatedtbis_sesdt<-array(NA,dim=c(rep,N,Npvalues))
data_cover_rate_bestmodel_semult<-array(NA,dim=c(rep,N,Npvalues))
data_cover_rate_bestmodel_sesdt<-array(NA,dim=c(rep,N,Npvalues))



file.to.load<-list.files()[regexpr("Results_",list.files())==1&regexpr("_Interceptmodel_V5.RData",list.files())>0&regexpr("_simplifiedanalysiscoverrate",list.files())>0  & regexpr(name_reduced,list.files())>0]
...
if (i<(rep+rep_to_add))
{
		added_elem<-matrix(NA,nrow=rep+rep_to_add-i,ncol=N)
		...
		added_elem<-array(NA,dim=c(rep+rep_to_add-i,N,Npvalues))
...
for (j in 1:N)
	...
	stock<-rbind(stock,c(i=i,j=j,N=N,firstdata=data[1],lastdata=data[dataSize]))
	}
	...
	ModelConsts=list(n=N, SE=se_estimates[i,])
	ModelData=list(y=mean_estimates[i,])
...
temp<-runMCMC_btadjust(code=modelCode_t, constants = ModelConsts, data = ModelData,params.save=params,params.conv=params,
	...
	res_Bayest[[i]]<-c(attributes(temp)[c("final.params","final.diags")],summary=list(summary(temp)))
	tempt<-temp
...
temp<-runMCMC_btadjust(code=modelCode_tbis, constants = ModelConsts, data = ModelData,params.save=params,params.conv=params,
	...
	res_Bayestbis[[i]]<-c(attributes(temp)[c("final.params","final.diags")],summary=list(summary(temp)))
	temptbis<-temp
...
temp<-runMCMC_btadjust(code=modelCode_Gaussian, constants = ModelConsts, data = ModelData,params.save=params,params.conv=params,
	...
	res_BayesGaus[[i]]<-c(attributes(temp)[c("final.params","final.diags")],summary=list(summary(temp)))
	...
	res_MCMC_dfs_tbis[[i]]<-MCMC_dfs_tbis
 ...
 for (j in 1:N)
	{ 
	data_cover_rate_gauss_semult[i,j]<-pnorm(0,mean=mean_estimates[i,j],sd=se_estimates[i,j])
	data_cover_rate_theort2_semult[i,j]<-pt(0-mean_estimates[i,j]/se_estimates[i,j],df=dataSize-length(coef(lModel))-1)
	data_cover_rate_theort1_semult[i,j]<-pt(0-mean_estimates[i,j]/se_estimates[i,j],df=dataSize-length(coef(lModel)))
	data_cover_rate_theort2_sesdt[i,j]<-pt(0-mean_estimates[i,j]/(se_estimates[i,j]*sqrt((dataSize-length(coef(lModel))-1-2)/(dataSize-length(coef(lModel))-1))),df=dataSize-length(coef(lModel))-1)
	data_cover_rate_theort1_sesdt[i,j]<-pt(0-mean_estimates[i,j]/(se_estimates[i,j]*sqrt((dataSize-length(coef(lModel))-2)/(dataSize-length(coef(lModel))))),df=dataSize-length(coef(lModel)))
	
	
	temp<-sapply(MCMC_dfs_t,function(x){pt(0-mean_estimates[i,j]/se_estimates[i,j],df=x)}    )
	temprand<-sample(temp,1)
	data_cover_rate_estimatedt_semult[i,j,]<-c(temp[1],temprand,mean(temp),pt(0-mean_estimates[i,j]/se_estimates[i,j],df=mean(MCMC_dfs_t)),pt(0-mean_estimates[i,j]/se_estimates[i,j],df=median(MCMC_dfs_t)))
	if((res_BayesGaus[[i]])$final.params$extra$DIC.mode>(res_Bayest[[i]])$final.params$extra$DIC.mode)
		{
		data_cover_rate_bestmodel_semult[i,j,]<-c(temp[1],temprand,mean(temp),pt(0-mean_estimates[i,j]/se_estimates[i,j],df=mean(MCMC_dfs_t)),pt(0-mean_estimates[i,j]/se_estimates[i,j],df=median(MCMC_dfs_t)))
		} else {
		data_cover_rate_bestmodel_semult[i,j,]<-rep(pnorm(0,mean_estimates[i,j],se_estimates[i,j]),5)
		
		}
	
	temp<-sapply(MCMC_dfs_tbis,function(x){pt(0-mean_estimates[i,j]/se_estimates[i,j],df=x)}    )
	temprand<-sample(temp,1)
	data_cover_rate_estimatedtbis_semult[i,j,]<-c(temp[1],temprand,mean(temp),pt(0-mean_estimates[i,j]/se_estimates[i,j],df=mean(MCMC_dfs_tbis)),pt(0-mean_estimates[i,j]/se_estimates[i,j],df=median(MCMC_dfs_tbis)))
	
	
	temp<-sapply(MCMC_dfs_t,function(x){pt(0-mean_estimates[i,j]/(se_estimates[i,j]*sqrt(x/(x-2))),df=x)}    )
	temprand<-sample(temp,1)
	data_cover_rate_estimatedt_sesdt[i,j,]<-c(temp[1],temprand,mean(temp),pt(0-mean_estimates[i,j]/(se_estimates[i,j]*sqrt(mean(MCMC_dfs_t)/(mean(MCMC_dfs_t)-2))),df=mean(MCMC_dfs_t)),pt(0-mean_estimates[i,j]/(se_estimates[i,j]*sqrt(median(MCMC_dfs_t)/(median(MCMC_dfs_t)-2))),df=median(MCMC_dfs_t)))
	if((res_BayesGaus[[i]])$final.params$extra$DIC.mode>(res_Bayest[[i]])$final.params$extra$DIC.mode)
		{
		data_cover_rate_bestmodel_sesdt[i,j,]<-c(temp[1],temprand,mean(temp),pt(0-mean_estimates[i,j]/(se_estimates[i,j]*sqrt(mean(MCMC_dfs_t)/(mean(MCMC_dfs_t)-2))),df=mean(MCMC_dfs_t)),pt(0-mean_estimates[i,j]/(se_estimates[i,j]*sqrt(median(MCMC_dfs_t)/(median(MCMC_dfs_t)-2))),df=median(MCMC_dfs_t)))
		} else {
		data_cover_rate_bestmodel_sesdt[i,j,]<-rep(pnorm(0,mean_estimates[i,j],se_estimates[i,j]),5)
		}
		
	temp<-sapply(MCMC_dfs_tbis,function(x){pt(0-mean_estimates[i,j]/(se_estimates[i,j]*sqrt(x/(x-2))),df=x)}    )
	temprand<-sample(temp,1)
	data_cover_rate_estimatedtbis_sesdt[i,j,]<-c(temp[1],temprand,mean(temp),pt(0-mean_estimates[i,j]/(se_estimates[i,j]*sqrt(mean(MCMC_dfs_tbis)/(mean(MCMC_dfs_tbis)-2))),df=mean(MCMC_dfs_tbis)),pt(0-mean_estimates[i,j]/(se_estimates[i,j]*sqrt(median(MCMC_dfs_tbis)/(median(MCMC_dfs_tbis)-2))),df=median(MCMC_dfs_tbis)))
	
	
	}

if (length(res_BayesGaus)==i&length(res_Bayest)==i&sum(!is.na(data_cover_rate_bestmodel_sesdt[i,,]))>0) {ended_i=TRUE}


if (ended_i)
{
	name<-paste0("rep",i-1,"_N",N,"_dataSize",dataSize,"_",Nchains,"chains","_Rhat",Rhat.max,"_neff",neff.min)
	name<-gsub("\\.","p",name)
	if (file.exists(paste0("Results_simplifiedanalysiscoverrate_",name,"_Interceptmodel_V5.RData"))) {
	  #Delete file if it exists
	  file.remove(paste0("Results_simplifiedanalysiscoverrate_",name,"_Interceptmodel_V5.RData"))
	}
	name<-paste0("rep",i,"_N",N,"_dataSize",dataSize,"_",Nchains,"chains","_Rhat",Rhat.max,"_neff",neff.min)
	name<-gsub("\\.","p",name)
	save(list=ls(),file=paste0("Results_simplifiedanalysiscoverrate_",name,"_Interceptmodel_V5.RData"))
}
